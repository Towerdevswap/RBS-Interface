...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://agam.exchange - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/agam.exchange)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://agam.exchange'}
          color={text === 'https://agam.exchange' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
